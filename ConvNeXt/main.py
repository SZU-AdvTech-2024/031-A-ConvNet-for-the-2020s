...
# 检查文件是否存�?
if os.path.exists('../dataset/cifar-100-python/train'):
    print("文件存在")
else:
    print("文件不存在?")
#%%
...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default='../dataset/cifar-100-python', type=str,
                        help='dataset path')
...
def main(args):
    ...
    if args.distributed:
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=2, find_unused_parameters=False)
